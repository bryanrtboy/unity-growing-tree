            ...
            if (_attractors.Count > 0)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < _extremities.Count; i++)
                    {
                        ...
                        //Sanity check for when a branch misses all targets and keeps growing
                        //Does not reset, but at least keeps an infinite length branch from forming...
                        if (end.y > _sanityCheck)
                        {
                            continue;
                        }
